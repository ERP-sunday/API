...
import { Dish, DishCategory, DishIngredientUnity } from '../mongo/models/dish.model';
...
export class FixturesService {
    ...
    private async insertIngredientFixtures(): Promise<string[]> {
        ...
        const ingredientsId = ingredients.map((ingredient) => ingredient._id.toString())
    ...
    private async insertDishFixtures(ingredientsId: string[]): Promise<void> {
        ...
        for (let index = 0; index < faker.number.int({ min: 1, max: 3 }); index++) {
            ingredients.push(
                {
                    ingredientId: ingredientsId[faker.number.int({ max: ingredientsId.length-1 })],
